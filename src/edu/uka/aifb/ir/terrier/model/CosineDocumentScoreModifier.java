...
public class CosineDocumentScoreModifier implements DocumentScoreModifier {

	static Logger logger = Logger.getLogger( CosineDocumentScoreModifier.class );
		...
		for( int i=0; i<rs.getExactResultSize(); i++ ) {
			...
			if( documentVectorSize == 0 ) {
				...
				for( int j=0; j<terms[0].length; j++ ) {
					documentVectorSize += terms[1][j] * terms[1][j];
	...
	public CosineDocumentScoreModifier clone() {
		return new CosineDocumentScoreModifier();
	}
